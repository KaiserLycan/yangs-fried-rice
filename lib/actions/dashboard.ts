...
/**
 * Get top rated products (per-product reviews).
 */
export async function getTopRatedProducts(branchId?: string): Promise<RankedProduct[]> {
  const supabase = createClient();
  ...
  return sorted.map(item => ({
    name: item.name,
    count: item.count, // Using count for UI as per mockup
    percentage: Math.round((item.average / 5) * 100) // 5 is max rating
  }));
}
