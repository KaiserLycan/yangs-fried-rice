...
3. **Database Schema & Seed** 
   ...
   **Clean up and add demo data** (needs `SUPABASE_SERVICE_ROLE_KEY` in `.env.local`):
   ```bash
   npm run db:cleanse                  # dry run: lists test/invalid rows it would remove or repair
   ...
   ```
   Demo accounts all use the password `YangsDemo2026!` (override with `SEED_PASSWORD`): manager `ramon.tan@yangs.ph`, staff `grace.lim@yangs.ph`, rider `jerome.bautista@yangs.ph`, customer `liza.reyes@yangs-demo.ph`.

   Names and addresses are stored as atomic columns (`first_name`/`last_name`; `building_no`/`street`/`barangay`/`city`/`zip_code`). `name` and `address_details` still exist for reading but are generated by the database — write the parts, never the combined column. Length limits live in `lib/validation/fields.ts` and are mirrored as CHECK constraints in `supabase/migrations/20260924000000_atomic_names_and_addresses.sql`.
